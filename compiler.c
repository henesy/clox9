...
//< set-panic-mode
  fprint(1, "[line %d] Error", token->line);
  ...
  if (token->type == TOKEN_EOF) {
    ...
    fprint(1, " at '%.*s'", token->length, token->start);
  }
...
static void emitByte(u8int byte) {
  writeChunk(currentChunk(), byte, parser.previous.line);
}
//< Compiling Expressions emit-byte
//> Compiling Expressions emit-bytes
static void emitBytes(u8int byte1, u8int byte2) {
  emitByte(byte1);
  emitByte(byte2);
}
//< Compiling Expressions emit-bytes
...
static int emitJump(u8int instruction) {
...
static u8int makeConstant(Value value) {
...
static u8int identifierConstant(Token* name) {
...
static int addUpvalue(Compiler* compiler, u8int index, bool isLocal) {
...
//< mark-local-captured
    return addUpvalue(compiler, (u8int)local, true);
  }
//> resolve-upvalue-recurse

  int upvalue = resolveUpvalue(compiler->enclosing, name);
  if (upvalue != -1) {
    return addUpvalue(compiler, (u8int)upvalue, false);
  }
//< resolve-upvalue-recurse
...
static u8int parseVariable(const char* errorMessage) {
...
static void defineVariable(u8int global) {
...
static u8int argumentList(void) {
  u8int argCount = 0;
...
static void dot(bool canAssign) {
  ...
  u8int name = identifierConstant(&parser.previous);
...
/* Global Variables read-named-variable < Local Variables named-local
  u8int arg = identifierConstant(&name);
*/
...
//< Calls and Functions infix-left-paren
  ...
  { nil,     nil,    PREC_NONE },       // TOKEN_LEFT_BRACE [big]
...
/* Compiling Expressions rules < Types of Values table-not
  { nil,     nil,    PREC_NONE },       // TOKEN_BANG
*/
//> Types of Values table-not
  { unary,    nil,    PREC_NONE },       // TOKEN_BANG
//< Types of Values table-not
/* Compiling Expressions rules < Types of Values table-equal
  { nil,     nil,    PREC_NONE },       // TOKEN_BANG_EQUAL
*/
//> Types of Values table-equal
  { nil,     binary,  PREC_EQUALITY },   // TOKEN_BANG_EQUAL
//< Types of Values table-equal
  { nil,     nil,    PREC_NONE },       // TOKEN_EQUAL
/* Compiling Expressions rules < Types of Values table-comparisons
  { nil,     nil,    PREC_NONE },       // TOKEN_EQUAL_EQUAL
  { nil,     nil,    PREC_NONE },       // TOKEN_GREATER
  { nil,     nil,    PREC_NONE },       // TOKEN_GREATER_EQUAL
  { nil,     nil,    PREC_NONE },       // TOKEN_LESS
  { nil,     nil,    PREC_NONE },       // TOKEN_LESS_EQUAL
*/
//> Types of Values table-comparisons
  { nil,     binary,  PREC_EQUALITY },   // TOKEN_EQUAL_EQUAL
...
//> Global Variables table-identifier
  { variable, nil,    PREC_NONE },       // TOKEN_IDENTIFIER
//< Global Variables table-identifier
...
//> parameters
  if (!check(TOKEN_RIGHT_PAREN)) {
    do {
      ...
      u8int paramConstant = parseVariable("Expect parameter name.");
...
static void method(void) {
  ...
  u8int constant = identifierConstant(&parser.previous);
//> method-body
...
//< Methods and Initializers class-name
  u8int nameConstant = identifierConstant(&parser.previous);
  declareVariable();
...
static void funDeclaration(void) {
  u8int global = parseVariable("Expect function name.");
  markInitialized();
...
static void varDeclaration(void) {
  u8int global = parseVariable("Expect variable name.");
...
/* Jumping Back and Forth for-statement < Jumping Back and Forth for-exitss
  consume(TOKEN_SEMICOLON, "Expect ';'.");
*/
//> for-exitss
  int exitssJump = -1;
  if (!match(TOKEN_SEMICOLON)) {
    ...
    exitssJump = emitJump(OP_JUMP_IF_FALSE);
...
//< loop-start
  ...
  int exitssJump = emitJump(OP_JUMP_IF_FALSE);
