...
static Value clockNative(int argCount, Value* args) {
  return NUMBER_VAL((double)cputime());
}
//< Calls and Functions clock-native
//> reset-stack
...
/* Calls and Functions runtime-error-temp < Calls and Functions runtime-error-stack
  ...
  usize instruction = frame->ip - frame->function->chunk.code - 1;
  int line = frame->function->chunk.lines[instruction];
*/
/* Types of Values runtime-error < Calls and Functions runtime-error-stack
  fprint(1, "[line %d] in script\n", line);
*/
...
//> look-for-existing-upvalue
  ...
  if (upvalue != nil && upvalue->location == local) return upvalue;
...
#define READ_SHORT() \
    (vm.ip += 2, (u16int)((vm.ip[-2] << 8) | vm.ip[-1]))
*/
#define READ_BYTE() (*frame->ip++)
#define READ_SHORT() \
    (frame->ip += 2, (u16int)((frame->ip[-2] << 8) | frame->ip[-1]))
//< Calls and Functions run
...
//> interpret-capture-upvalues
        for (int i = 0; i < closure->upvalueCount; i++) {
          u8int isLocal = READ_BYTE();
          u8int index = READ_BYTE();
          if (isLocal) {
