...
static int constantInstruction(const char* name, Chunk* chunk,
                               int offset) {
  u8int constant = chunk->code[offset + 1];
...
static int invokeInstruction(const char* name, Chunk* chunk,
                                int offset) {
  u8int constant = chunk->code[offset + 1];
  u8int argCount = chunk->code[offset + 2];
...
static int byteInstruction(const char* name, Chunk* chunk, int offset) {
  u8int slot = chunk->code[offset + 1];
  print("%-16s %4d\n", name, slot);
...
static int jumpInstruction(const char* name, int sign, Chunk* chunk,
                           int offset) {
  u16int jump = (u16int)(chunk->code[offset + 1] << 8);
...
//< show-location
  ...
  u8int instruction = chunk->code[offset];
  switch (instruction) {
...
//> Closures disassemble-closure
    case OP_CLOSURE: {
      offset++;
      u8int constant = chunk->code[offset++];
      print("%-16s %4d ", "OP_CLOSURE", constant);
      printValue(chunk->constants.values[constant]);
      print("\n");
